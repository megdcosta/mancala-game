...
public class MancalaDataStructure implements Serializable{
    private static final long serialVersionUID = -2838822969582429640L;
    ...
    public int getPitNum(Countable pit) {
        return data.indexOf(pit)+1;
    }
    ...
    private int pitPos(int pitNum) {
        ...
        if(pitNum<1 || pitNum > 12){
            throw new RuntimeException("Pit Number Out of Range " + pitNum);
        }
        int pos = pitNum;
        if (pos <= PLAYER_ONE) {
            pos--;
        }
        return pos; //return pos-1 for player1 and pos for player2
    }
    ...
    private int storePos(int playerNum) {
        ...
        return pos; //return 6 for store1 and 13 for store2
    }
    ...
    private void loopIterator() {
        if (iteratorPos == PLAYER_TWO + 1) { //if iteratorPos == 14
            iteratorPos = 0;
        }
    }

    private void skipPosition() {
        while (iteratorPos == playerSkip || iteratorPos == pitSkip) { //while iteratorPos == 13 or == -1
            iteratorPos++;
            loopIterator();
        }
    }
